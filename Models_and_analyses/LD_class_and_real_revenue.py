# -*- coding: utf-8 -*-
"""
Created on Tue Jun 18 18:20:44 2024
...
class LT_Battery:
    def __init__(self, OptLen):
        ...
        #self.d_U = cp.Variable(self.OptLen,nonneg=True)
        #self.d_D = cp.Variable(self.OptLen,nonneg=True)
        ...
        #placeholder objective (see real objective under activation)
        self.obj = cp.Maximize(0)               
        self.constraints = [self.c_deg == -( (self.e["c"]+self.A_D)*self.Eta_up + (self.e["d"] + self.A_U)*self.Eta_down)/(2*self.E_max) * (self.capex/self.CycleLife),
                            ...
                            #self.d_U - self.d_D == self.A_U - self.A_D,
                            ...
                            self.e["c"][0] + self.p["FCRN"][0] + self.p["FCRD_D"][0] + self.p["aFRR_D"][0] <= self.P_max,                                    #charging cannot exceed power cap. Some cap is reserved for FCR.
                            self.e["d"][0] + self.p["FCRN"][0] + self.p["FCRD_U"][0] + self.p["aFRR_U"][0] + self.p["mFRR_U"][0] <= self.P_max,                           #discharging cannot exceed power cap. Some cap is reserved for FCR.
                            
                            #C-rate is multiplied on previous hour's reservation
                            (self.e["c"][1:] + self.p["FCRN"][1:] + self.p["FCRD_D"][1:] + self.p["aFRR_D"][1:]) + (self.k)*(1/3*self.p["FCRD_U"][:-1] + self.p["FCRN"][:-1] + self.p["aFRR_U"][:-1] + self.p["mFRR_U"][:-1])*self.Eta_down/self.Eta_up <= self.P_max,
                            (self.e["d"][1:] + self.p["FCRN"][1:] + self.p["FCRD_U"][1:] + self.p["aFRR_U"][1:] + self.p["mFRR_U"][1:]) + (self.k)*(1/3*self.p["FCRD_D"][:-1] + self.p["FCRN"][:-1] + self.p["aFRR_D"][:-1])*self.Eta_up/self.Eta_down <= self.P_max,#*self.Eta_up,
                            
                            #(self.e["d"][1:] + self.p["FCRN"][1:] + self.p["FCRD_U"][1:] + self.p["aFRR_U"][1:] + self.p["mFRR_U"][1:])*self.Eta_down <= self.P_max*self.Eta_up,
...
#FCRN: LER units/portfolios must have storage capacity of minimum 1 hour to handle long lasting frequency deviations. file:///C:/Users/filip/Downloads/ancillary-services-to-be-delivered-in-denmark-tender-conditions-1-2-2024%20(1).pdf
    def set_params(self,P_max,E_max,E_min,Eta_up,Eta_down,Eta_lin,E_start,cl,cpx,et,etp):
    ...
    def set_avg_act(self,act):
        ...
        self.obj =  cp.Maximize(#self.p["DA"]
                               ...
                               + cp.sum(self.r_act) - cp.sum(self.c_deg) #+ cp.sum(self.r_rec)
                               #cycle cost - note 
    ...
    def solve(self):
        ...
        self.prob.solve(solver=cp.SCIP,verbose=False)#solver=cp.SCIP
        if self.prob.status != 'optimal':
            print(str(self.prob.status))
        return self.prob.value
    def res(self,hours):
        r = pd.DataFrame({"p_DA":self.p["DA"].value[:hours],
                "p_FCRN":    self.p["FCRN"].value[:hours],
                "p_FCRD_U":  self.p["FCRD_U"].value[:hours],
                "p_FCRD_D":  self.p["FCRD_D"].value[:hours],
                "p_aFRR_U":  self.p["aFRR_U"].value[:hours],
                "p_aFRR_D":  self.p["aFRR_D"].value[:hours],
                "p_mFRR_U":  self.p["mFRR_U"].value[:hours],
                "e_b":       self.e["b"].value[:hours],
                "e_c":       self.e["c"].value[:hours],
                "e_d":       self.e["d"].value[:hours]})        
        return r
    ...
    def run_yr(self,yrP,yrA,start_d,end_d):        
        ...
        for i in range(len(optStart)):
            ...
            #if sum( (np.array(self.d_D.value)>0.1) *(np.array(self.d_U.value)>0.1) ) > 0:
            #    print(self.d_D.value[(np.array(self.d_D.value)>0.1) *(np.array(self.d_U.value)>0.1)])
            #    print(self.d_U.value[(np.array(self.d_D.value)>0.1) *(np.array(self.d_U.value)>0.1)])
...
class FCR_LT_Battery:
    def __init__(self, OptLen):
        ...
        self.constraints = [self.c_deg == -( (self.e["c"]+self.A_D)*self.Eta_up + (self.e["d"] + self.A_U)*self.Eta_down)/(2*self.E_max) * (self.capex/self.CycleLife),
                            ...
                            #C-rate is multiplied on previous hour's reservation
                            (self.e["c"][1:] + self.p["FCRN"][1:] + self.p["FCRD_D"][1:] + self.p["aFRR_D"][1:]) + (self.k)*(1/3*self.p["FCRD_U"][:-1] + 0.4*self.p["FCRN"][:-1] + self.p["aFRR_U"][:-1] + self.p["mFRR_U"][:-1])*self.Eta_down/self.Eta_up <= self.P_max,
                            (self.e["d"][1:] + self.p["FCRN"][1:] + self.p["FCRD_U"][1:] + self.p["aFRR_U"][1:] + self.p["mFRR_U"][1:]) + (self.k)*(1/3*self.p["FCRD_D"][:-1] + 0.4*self.p["FCRN"][:-1] + self.p["aFRR_D"][:-1])*self.Eta_up/self.Eta_down <= self.P_max,#*self.Eta_up,
...
def dcf(rev,LT,r_DC,r_T,OPEX,CAPEX): #yrRev,inflation
    ...
    #Revenue = pd.DataFrame({"REAL":np.ones(LT)*yrRev:
    #                        "NOMINAL":})
...
def yr_rev_cyc_LT(bat,res,yrA,yrP,TC,TP):
    yrP = yrP["2023"]
    ...
    for i in range(1,len(Real_SOE)-1):
        ...
        #if 1:#m > 2*1/C:
        if res["e_b"][i] - Real_SOE[i] < -C*E: 
            rec_d[i+1] = max( ( res["e_b"][i] - Real_SOE[i])*(1/Eta_down),max_d[i+1])
        elif res["e_b"][i] - Real_SOE[i] > C*E: 
